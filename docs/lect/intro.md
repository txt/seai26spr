...
Finding authoritative labels remains a major challenge for
MSR-style research [5, 32, 39, 64, 65, 69, 75]:

**Human experts**: Possible but slow and error-prone when
rushed [23], often taking hours for just a few cases
[41, 45, 66]

**Historical logs**: Provide large label sets but are
frequently unreliable:

- 90% of technical debt "false positives" were incorrect [75]
- Similar issues occur in security [69], static analysis [40],
  and defect data [61]

**Automated labeling**: Also faces limits:

- Regex-based heuristics are crude [39]
- LLMs are only assistive (but not authoritative)

**Natural oracles**: Some domains have naturally occurring
oracles (e.g. compile with certain Makefile settings then run
the full test suite) but these can be extremely slow

- x264 example: 1,536 hours to fully explore [67]

#### BASELINE Clustering Experiment

When we cannot trust many labels, we must do what we can with
very few labels. The rq2.sh script of MOOT clusters data,
labels a few items per cluster, then uses that information to
guess the labels of nearby items.

**The method:**

After dividing data 50:50 into train and hold-out:

**TRAIN a model:**

1. Label, say, <tt>n<sub>1</sub> = 30</tt> randomly selected
   training examples
2. Sort them on <tt>y</tt> (the goals)
3. Divide sorted data into a <tt>&radic;n</tt> best set and a
   <tt>n - &radic;n</tt> rest set
4. Return Model(row) = <tt>dist(row, C<sub>b</sub>) - dist(
   row, C<sub>r</sub>)</tt> where <tt>C<sub>b</sub></tt> and
   <tt>C<sub>r</sub></tt> are the centroids on best and rest
   sets; and <tt>dist</tt> is Euclidean distance between
   <tt>x</tt> values

**TEST a model on hold-outs:**

1. Sort the hold-out on <tt>y</tt> using Model
2. Label the first, say, <tt>n<sub>2</sub> = 10</tt> items in
   that sort
3. Return the best of these as the recommendation

Note that this process uses only <tt>n = n<sub>1</sub> + n
<sub>2</sub></tt> labels, which in this case, was
<tt>30 + 10 = 40</tt>.

#### The Results

Figure 3 from [49] compares using only <tt>n</tt> labels
versus all <tt>N > n</tt>, evaluated with nonparametric tests
(Kolmogorov–Smirnov for significance and Cliff's Delta for
effect size).

**BLUE curve**: The percent frequency where <tt>x < 160</tt>
samples performed statistically as well as 160 samples.

**RED curve**: The performance of the row recommended by
BASELINE after <tt>x</tt> samples. This number is normalized
such that <tt>y = 0</tt> means "same as the untreated mean
goal score" (i.e. no improvement); and <tt>y = 100</tt> means
"best possible row found".

**Key findings:**

- The BLUE curve shows the usual story: more labels
  (<tt>N > n</tt>) wins statistically
- But the RED curve shows something unexpected: after about 40
  labels, there is very little further improvement (from 68 to
  73)
- After ~40 labels, **diminishing returns**

#### The Insight

For mission-critical and safety-critical applications, that
5-point improvement may be important. But for many other
applications, Figure 3 suggests that a mere **40 labels could
suffice**.

**That's a 4× cost reduction.**

**Note:** This result runs contrary to much of the current
fashion in AI since it says **more data is not always better**.

If this holds for other optimizers (and not just this
simplistic BASELINE method) then this would be an important
insight for resource-limited processes such as:

- Cutting cloud costs
- Deploying on the edge
- Avoiding fatigue in human-in-the-loop labeling
